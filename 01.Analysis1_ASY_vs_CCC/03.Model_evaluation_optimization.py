...
# 1. Load data
	...
	# 1.1. Train
rawDataTrain = pd.read_csv("01.Analysis1_ASY_vs_CCC/01.Feature_selection_percent_methylation/ASY_vs_CCC_train.txt", sep = "\t")
y_train = rawDataTrain.iloc[:,-1]
X_train = rawDataTrain.iloc[:,0:-1]

	# 1.2. Test
rawDataTest = pd.read_csv("01.Analysis1_ASY_vs_CCC/01.Feature_selection_percent_methylation/ASY_vs_CCC_test.txt", sep = "\t")
...
rfe = RFE(estimator=svm.SVC(kernel = "linear"), n_features_to_select=35, importance_getter='coef_')
...
model = svm.SVC(kernel = bestP['kernel'], C = bestP['C'], gamma = bestP['gamma'], class_weight = 'balanced', probability=True)
...
df.to_csv("01.Analysis1_ASY_vs_CCC/03.Model_evaluation_optimization/ASY_CCC_prediction.tab", sep = "\t")
...
plt.legend()
plt.savefig('01.Analysis1_ASY_vs_CCC/03.Model_evaluation_optimization/ROC_curve.pdf')
