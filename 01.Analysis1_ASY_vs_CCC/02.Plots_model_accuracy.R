...
for(i in algos){
    # We read the data
    data <- read.table(paste("01.Analysis1_ASY_vs_CCC/02.Feature_selection_machine_learning/ASY_CCC_", i, ".txt", sep = ""), sep = "\t", header = TRUE)
    # We rename the algo
    if(i == "DecisionTree"){
...
png("01.Analysis1_ASY_vs_CCC/02.Feature_selection_machine_learning/ASY_vs_CCC_model_evaluation.png", res = 300, width = 8, height = 6, units = "in")
plot(p)
dev.off()
